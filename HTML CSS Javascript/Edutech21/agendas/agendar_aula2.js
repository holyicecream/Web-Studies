...
var listaAulas2 = JSON.parse(localStorage.getItem("listaAulas2")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas2.length >= 2) {
  ...
  for (var i = 0; i < listaAulas2.length; i++) {
    var aula = listaAulas2[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas2", JSON.stringify(listaAulas2));
