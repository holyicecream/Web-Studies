...
var listaAulas8 = JSON.parse(localStorage.getItem("listaAulas8")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas8.length >= 2) {
  ...
  if (listaAulas8.length === 0) {
    tabelaAulas.innerHTML = '<tr><td colspan="3">Nenhum evento</td></tr>';
    return;
  }
  tabelaAulas.innerHTML = "";
  for (var i = 0; i < listaAulas8.length; i++) {
    var aula = listaAulas8[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas8", JSON.stringify(listaAulas8));
