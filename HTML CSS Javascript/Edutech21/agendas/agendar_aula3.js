...
var listaAulas3 = JSON.parse(localStorage.getItem("listaAulas3")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas3.length >= 2) {
  ...
  if (listaAulas3.length === 0) {
    tabelaAulas.innerHTML = '<tr><td colspan="3">Nenhum evento</td></tr>';
    return;
  }
  tabelaAulas.innerHTML = "";
  for (var i = 0; i < listaAulas3.length; i++) {
    var aula = listaAulas3[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas3", JSON.stringify(listaAulas3));
