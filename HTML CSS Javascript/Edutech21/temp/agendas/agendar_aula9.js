...
var listaAulas9 = JSON.parse(localStorage.getItem("listaAulas9")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas9.length >= 2) {
  ...
  if (listaAulas9.length === 0) {
    tabelaAulas.innerHTML = '<tr><td colspan="3">Nenhum evento</td></tr>';
    return;
  }
  tabelaAulas.innerHTML = "";
  for (var i = 0; i < listaAulas9.length; i++) {
    var aula = listaAulas9[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas9", JSON.stringify(listaAulas9));
