...
var listaAulas6 = JSON.parse(localStorage.getItem("listaAulas6")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas6.length >= 2) {
  ...
  if (listaAulas6.length === 0) {
    tabelaAulas.innerHTML = '<tr><td colspan="3">Nenhum evento</td></tr>';
    return;
  }
  tabelaAulas.innerHTML = "";
  for (var i = 0; i < listaAulas6.length; i++) {
    var aula = listaAulas6[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas6", JSON.stringify(listaAulas6));
