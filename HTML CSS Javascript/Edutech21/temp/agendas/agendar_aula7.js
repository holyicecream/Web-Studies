...
var listaAulas7 = JSON.parse(localStorage.getItem("listaAulas7")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas7.length >= 2) {
  ...
  if (listaAulas7.length === 0) {
    tabelaAulas.innerHTML = '<tr><td colspan="3">Nenhum evento</td></tr>';
    return;
  }
  tabelaAulas.innerHTML = "";
  for (var i = 0; i < listaAulas7.length; i++) {
    var aula = listaAulas7[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas7", JSON.stringify(listaAulas7));
