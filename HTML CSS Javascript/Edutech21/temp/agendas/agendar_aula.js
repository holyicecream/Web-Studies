...
var listaAulas = JSON.parse(localStorage.getItem("listaAulas")) || [];
...
function atualizarTabelaAulas() {
  ...
  for (var i = 0; i < listaAulas.length; i++) {
    var aula = listaAulas[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas", JSON.stringify(listaAulas));
