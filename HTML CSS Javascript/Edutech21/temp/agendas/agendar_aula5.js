...
var listaAulas5 = JSON.parse(localStorage.getItem("listaAulas5")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas5.length >= 2) {
  ...
  if (listaAulas5.length === 0) {
    tabelaAulas.innerHTML = '<tr><td colspan="3">Nenhum evento</td></tr>';
    return;
  }
  tabelaAulas.innerHTML = "";
  for (var i = 0; i < listaAulas5.length; i++) {
    var aula = listaAulas5[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas5", JSON.stringify(listaAulas5));
