...
var listaAulas4 = JSON.parse(localStorage.getItem("listaAulas4")) || [];

function atualizarTabelaAulas() {
  
  if (listaAulas4.length >= 2) {
  ...
  if (listaAulas4.length === 0) {
    tabelaAulas.innerHTML = '<tr><td colspan="3">Nenhum evento</td></tr>';
    return;
  }
  tabelaAulas.innerHTML = "";
  for (var i = 0; i < listaAulas4.length; i++) {
    var aula = listaAulas4[i];
...
function salvarNovaAula(event) {
  ...
  if (novaAula(titulo, turma, data, materia, desc, senha)) {
    ...
    localStorage.setItem("listaAulas4", JSON.stringify(listaAulas4));
